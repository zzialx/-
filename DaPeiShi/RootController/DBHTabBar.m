...
- (void)layoutSubviews
{
    ...
    // 1.设置加号按钮的位置
    CGFloat distance = 0;
    if (CGRectGetHeight(self.frame) >49) {
        distance = CGRectGetHeight(self.frame) - 49;
    }
    self.plusButton.center = CGPointMake(CGRectGetWidth(self.frame) * 0.5, (CGRectGetHeight(self.frame) -distance) * 0.5);
