    ...
    if (webSocket == self.socket) {
        ...
        NSString *string = [NSString stringWithFormat:@"%@",dicData[@"code"]];
            if (dicData && [dicData isKindOfClass:[NSDictionary class]]) {
                if ([string  isEqual:@"5101" ]) {//9990
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"MESSAGE_NOTICE" object:dicData[@"bean"][@"messageCountVo"]];
                } else if ([string  isEqual:@"6002"]){
                     [[NSNotificationCenter defaultCenter] postNotificationName:@"COMEIN" object:dicData[@"bean"]];
                }
            }
    }
}
