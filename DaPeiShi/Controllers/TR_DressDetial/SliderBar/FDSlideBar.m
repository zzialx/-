...
- (void)setupItems {
    ...
    for (NSString *title in _itemsTitle) {
        ...
        CGFloat item_width = [title boundingRectWithSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(_scrollView.frame))
                                         options:NSStringDrawingUsesLineFragmentOrigin
                                      attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]}
                                         context:nil].size.width;
        
        item.frame = CGRectMake(itemX, 0, item_width, CGRectGetHeight(_scrollView.frame));
        ...
        itemX = CGRectGetMaxX(item.frame)+10;
    }
    ...
    CGFloat item_width = [firstItem.title boundingRectWithSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(_scrollView.frame))
                                             options:NSStringDrawingUsesLineFragmentOrigin
                                          attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]}
                                             context:nil].size.width;
    //    CGFloat itemW = [FDSlideBarItem widthForTitle:_selectedItem.title];
    // Set the frame of sliderView by the selected item
    _sliderView.frame = CGRectMake((item_width-4)/2+16, self.frame.size.height - 4, 4, 4);
}
...
- (void)addAnimationWithSelectedItem:(FDSlideBarItem *)item isYes:(BOOL)isYes{
    ...
    _sliderView.layer.position = CGPointMake(item.centerX-2, _sliderView.layer.position.y);
