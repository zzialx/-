...
@interface TR_PublichDressPicVC ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextViewDelegate,AMapLocationManagerDelegate,ZYQAssetPickerControllerDelegate>
...
@implementation TR_PublichDressPicVC

-(void)viewDidLoad{
    [super viewDidLoad];
    self.view.backgroundColor = UIColor.whiteColor;
//    [self.navView setLeftImg:@"" title:@"" bgColor:UIColor.whiteColor];
    [self.navView.leftBtn setTitle:@"取消" forState:UIControlStateNormal];
    [self.navView.leftBtn setTitleColor:UIColor.blackColor forState:UIControlStateNormal];
...
#pragma mark - 地理定位2个代理方法
- (void)amapLocationManager:(AMapLocationManager *)manager didUpdateLocation:(CLLocation *)location{
    NSLog(@"我是个倒霉蛋，我不会被调用");
}
- (void)amapLocationManager:(AMapLocationManager *)manager didFailWithError:(NSError *)error{
    NSLog(@"定位错误信息===%@",error);
    [TRHUDUtil showMessageWithText:error.description];
}
...
-(void)click{
    ...
    pickerController.maximumNumberOfSelection = 9-self.imageArray.count;
